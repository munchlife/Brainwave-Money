...
module.exports = function() {
        ...
        return rest.postJson(url, data)
            .then(function(result) {
                ...
                var signalPathwayData = {
                  /*signalPathwayId: 0,*/
                    pheromone:                            result.data.access_token,
                    wavePheromoneExpiration:              new Date(new Date().getTime() + ((result.data.expire_in-3)*1000)),
                ...
                var data = {
                    access_token: signalPathwayData.pheromone // access_token
                };
    ...
    self.refreshTokens = function(signalPathway) {
        ...
        if (signalPathway.wavePheromoneExpiration === null || signalPathway.wavePheromoneExpiration > new Date()) {
            ...
            rest.postJson(url, data)
                .then(function(result) {
                    ...
                    signalPathway.pheromone                            = result.access_token;
                    signalPathway.wavePheromoneExpiration              = new Date(new Date().getTime() + ((result.expires_in-3)*1000));
    ...
    // -------------------------------------------------------------------------
    self.account = function(signalPathway) {
        verbose('#account()');
        ...
        var options = {
            query: {
                access_token: signalPathway.pheromone // access_token
            }
        };
    ...
    // -------------------------------------------------------------------------
    self.messages = function(signalPathway) {
        verbose('#messages()');
        if (signalPathway.optional === null)
            return Promise.reject(new Error('appleID account ID not available'));
    ...
    self.send = function(signalPathways, message) {
        ...
        if (signalPathways.cell.pheromone !== null)
        ...
        var options = {
            query: {
                access_token: signalPathways.life.pheromone
            },
            data: {
